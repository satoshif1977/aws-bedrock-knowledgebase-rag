...
Bedrock Knowledge Bases（Retrieve / RetrieveAndGenerate）の ThrottlingException や
一時的なサーバエラーに対して、AWS 公式推奨の「指数バックオフ + フルジッター」方式で
自動リトライする。
...
使い方:
    result = retry_call(bedrock_agent_runtime.retrieve, knowledgeBaseId=..., ...)
